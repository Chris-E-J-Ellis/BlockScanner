namespace BlockScanner.UnitTests
{
    ...
    public class DynamicRegionBitmapProviderTests
    {
        ...
        public void BenchMark_MultipleOverlappingRegionsManyThreads()
        {
            ...
            var captureOne = new Task<string>(() => TimerLoop(() => provider.CaptureScreenRegion(rectOne), 200, "One"));
            var captureTwo = new Task<string>(() => TimerLoop(() => provider.CaptureScreenRegion(rectTwo), 200, "Two"));
            var captureThree = new Task<string>(() => TimerLoop(() => provider.CaptureScreenRegion(rectThree), 200, "Three"));
            var captureFour = new Task<string>(() => TimerLoop(() => provider.CaptureScreenRegion(rectThree), 200, "Four"));
